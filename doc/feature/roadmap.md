...
- [ ] **工作区锚点**：IDE 工作区 ↔ DSH workspace 的会话级联动（G-02）
...
### M1 会话管理（v0.2.x）— [01-session-management](01-session-management/)
- [ ] **G-12 会话管理器侧边栏**：顶部进程状态 + `＋新建会话` + 会话列表（`✕` 关标签页、显示模型名、同工作区不显 cwd）→ 细节见 [01-session-management/req.md](01-session-management/req.md)
- [ ] **G-02 工作区切换联动**：DSH workspace 锚点随 IDE 切，不杀进程、reload 自动恢复 → 细节见 [01-session-management/req.md](01-session-management/req.md)
- [ ] **G-03 dsh 版本软校验**：旧版本 UI 警告 → 细节见 [01-session-management/req.md](01-session-management/req.md)
...
```
架构（桥/进程/UI，✅）→ M1 会话管理 → M2 IDE 上下文 → M3 编排
非功能项持续并行
```
