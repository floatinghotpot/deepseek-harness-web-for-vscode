...
### M2 多会话管理（v0.2.0）— 02-session-management ✅
- [x] **G-12 会话管理器侧边栏**：全部活跃会话列表（标题/相对时间/`✎` 重命名/`✕` 归档）+ 可展开归档区（标题/时间）+ 空白会话 "New Session" 行；5s 轮询（state 感知 + 防重入）
- [x] **多面板绑定多会话**：`＋新建会话` → `session.create` → 新面板（堆叠打开、reload 恢复、tab 标题联动、归档关面板）

### M3 升级渠道与 rc.8 兼容（v0.3.0）— 03-upgrade-channels ✅
- [x] **R1 `--no-open` 版本门控**：spawn 仅在 dsh ≥ 0.1.0-rc.8 时追加 `--no-open`（rc.8 起 `dsh web` 默认弹外部浏览器，嵌入式 UI 场景需抑制；旧版本不认此 flag 会启动失败，故按版本门控）
- [x] **R2 双渠道升级**：同时跟踪 npm `latest`/`next` dist-tag；侧边栏按渠道显示两个升级按钮（`@latest`/`@next` 命令，QuickPick 预填不执行）
...
- [ ] **G-04 CI 自动双发**（tag 触发发布，模板在 doc/publish/github-actions.md）
...
```
架构（桥/进程/UI，✅）→ M1 工作区对齐（✅）→ M2 多会话管理（✅）→ M3 升级渠道（✅）→ M4 IDE 上下文 → M5 编排
非功能项持续并行
```
