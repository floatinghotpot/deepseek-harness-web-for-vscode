...
- **VSIX**：`make package`（当前 25+ 文件，含 icon.png/README/README.zh/LICENSE/nls）
...
## 零、快速参考（Makefile 已封装，推荐）

- `make package` — 编译 + 打 vsix（= `npm run compile` + `npm run package`）
- `make publish-vscode` — 发布 Marketplace（需要 `VSCE_PAT`，或已 `vsce login`）
- `make publish-ovsx` — 发布 Open VSX（需要 `OVSX_TOKEN`，~/.zshrc 已导出）
- `make publish` — **双渠道一次发布**（= package + publish-vscode + publish-ovsx）
- `make namespace` — 创建 Open VSX namespace（一次性）
- `make tag` — 打 git tag `v<version>`（不 push）

tokens 只从环境变量读取、命令以 `@` 开头**不回显**（Makefile 头注释）；缺 token 时 fail loudly（见各目标）。底层手动 npx 命令见下文各节（供排查/未装 make 时用）。
...
# 2) 一键打包 + 发布（Makefile；token 从环境变量读，零回显）
make publish-vscode
# 底层等价（手动命令，供排查）：
#   make package
#   npx --no-install vsce publish --packagePath deepseek-harness-web-for-vscode-<ver>.vsix
```
...
### 2. 发布
```sh
make publish-ovsx   # 需要 OVSX_TOKEN（~/.zshrc 已导出）
# 底层等价（手动命令，供排查）：
#   npx --yes ovsx publish deepseek-harness-web-for-vscode-<ver>.vsix -p "$OVSX_TOKEN"
```
...
- **发布一律走 Makefile**（`make publish-vscode` / `make publish-ovsx` / `make publish`），不手敲 npx——自动带 token 校验与零回显；底层命令见各节"供排查"注释；
