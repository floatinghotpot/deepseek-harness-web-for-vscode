...
<body>
  <div class="header">
    ...
    <div>
      ...
      <div class="subtitle">${t("launcher.subtitle")}</div>
    </div>
  </div>
  ...
  <div class="actions">
    <button class="primary" id="start" style="display:${showStart ? "inline-block" : "none"}">${t("button.start")}</button>
...
(function(){
  ...
  window.addEventListener("message", function (e) {
    ...
    else if (m.state === "ready") set("ready", ${JSON.stringify(t("launcher.ready", { url: "{url}" }))}.replace("{url}", m.url || ""));
