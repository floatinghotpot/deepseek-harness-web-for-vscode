    ...
    init.state === "ready"
      ...
      : init.state === "starting"
        ...
        : init.state === "stopping"
          ? "停止中…"
          : init.state === "error"
            ? "错误：" + (init.message ?? "未知错误")
            : "未启动";
  const showStart = init.state === "stopped" || init.state === "error";
  const showReady = init.state === "ready";
  const workspaceText = init.workspaceName ? "工作区: " + init.workspaceName : "未打开文件夹 — DSH 将使用主目录";
...
<body>
  <div class="header">
    ...
    <div>
      ...
      <div class="subtitle">启动并内嵌 DSH Web UI</div>
    </div>
  </div>
  ...
  <div class="actions">
    <button class="primary" id="start" style="display:${showStart ? "inline-block" : "none"}">启动 DeepSeek Harness</button>
    <div class="row" id="readyActions" style="display:${showReady ? "flex" : "none"}">
      <button class="secondary" id="openPanel">打开面板</button>
...
(function(){
  ...
  function set(state, text) {
    ...
    start.style.display = state === "stopped" || state === "error" ? "inline-block" : "none";
  ...
  window.addEventListener("message", function (e) {
    ...
    if (m.type === "workspace") {
      footer.textContent = m.name ? "工作区: " + m.name : "未打开文件夹 — DSH 将使用主目录";
      return;
    }
    if (m.type !== "server-status") return;
    if (m.state === "stopped") set("stopped", "未启动");
    else if (m.state === "starting") set("starting", "启动中…");
    else if (m.state === "stopping") set("starting", "停止中…");
    else if (m.state === "ready") set("ready", "运行中 " + (m.url || ""));
    else if (m.state === "error") set("error", "错误：" + (m.message || "未知错误"));
  });
})();
</script>
</body>
